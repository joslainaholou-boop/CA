# Semaine 3 — brancher le tri automatique (13 arrondissements)
...
votre Google Drive. Le déploiement couvre désormais les 13 arrondissements de
la commune de Ouidah : 13 numéros niveau 1 (un par arrondissement) partagent
...
2. Cliquez sur le **+** en bas pour créer un second onglet, nommé **`Chefs`**,
   ...
   les 13 arrondissements et leurs 77 quartiers/villages — copiez-collez son
...
2. Ajouter un crédit de facturation (à l'échelle des 13 arrondissements,
...
## 2. Récupérer les identifiants WhatsApp (Meta) — pour les 13 numéros

Sur **developers.facebook.com**, dans l'app créée en semaine 2 :

1. Onglet **WhatsApp → Configuration de l'API** : le numéro OUIDAH III y
   figure déjà. Ajoutez les 12 autres numéros niveau 1 un par un
...
2. Générer **un seul jeton d'accès permanent** valable pour les 13 numéros
...
3. Icône **⚙️ Paramètres du projet** → **Propriétés du script → Ajouter une
   ...
   | `WHATSAPP_TOKEN` | le jeton permanent obtenu à l'étape 2 (valable pour les 13 numéros) |
   ...
   message, lequel des 13 numéros l'a reçu.

## 4. Déployer le script comme site web (une seule fois pour les 13 numéros)
...
## 5. Brancher le webhook côté Meta — pour chacun des 13 numéros
...
3. Valider, puis cocher **messages**. Ce réglage de webhook est partagé par
   les 13 numéros de l'app — pas besoin de le refaire pour chacun.
...
5. Répéter le test pour 2 ou 3 autres arrondissements avant d'ouvrir aux 13.
...
2. Fonction à exécuter : `dailySynthesis`. Source de l'événement :
   **Déclencheur horaire**. Type : **Minuteur jour**, à l'heure souhaitée
   (ex. 8h à 9h).
3. Enregistrer. La synthèse envoyée à la mairie regroupe désormais les 13
   arrondissements, avec un décompte par arrondissement.

## Recommandation de déploiement progressif

Avec 13 arrondissements et 77 quartiers/villages, mieux vaut ne pas ouvrir
les 13 numéros le même jour : activez et testez OUIDAH III seul (déjà fait),
